...
class Quotex:
    ...
    async def get_candles_deep(self, asset, amount_of_seconds, period, timeout=DEFAULT_TIMEOUT, progress_callback=None):
        ...
        :param asset: Asset code (e.g., "USDPKR_otc")
        :param amount_of_seconds: How much history to fetch into the past (in seconds)
        :param period: Timeframe per candle (e.g., 60 for 1 minute)
        :returns: List of stitched and sorted candle dicts.
        """
        import json
        all_candles = {}
        ...
        # 1. Fetch initial baseline batch using standard method
        candles = await self.get_candles(asset, current_time, chunk_offset, period, timeout=timeout)
        if not candles:
            return []
        ...
        while oldest_time > target_oldest_time:
            ...
            # Reset internal parser caches for new payload collision
            self.api.candles.candles_data = None
            ...
            # Smart-wait for data to arrive in v2 pipeline
            start_time = time.time()
            v2_data = None
            while time.time() - start_time < timeout:
                v2_data = self.api.candle_v2_data.get(asset)
                # Ensure the data packet matches the exact index we just requested
                if v2_data and v2_data.get("index") == browser_index:
                    break
                await asyncio.sleep(0.1)
                
            if not v2_data or v2_data.get("index") != browser_index:
                logger.warning(f"Failed or timed out fetching deep history at chunk {fetch_time}")
                break
